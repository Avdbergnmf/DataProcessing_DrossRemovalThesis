...
'''
This script contains the main class which loads all data, does all the calculations,
creates and saves the plots of the data, and prints the results to txt files.

To perform these calculations, the main script is written on the bottom of this
file. It contains an explanation of all the settings for the DataProcessing
class.
------------------------------------------------------------------------------
For additional explanation of this script, you may email:
    Alex_van_den_berg@hotmail.com
------------------------------------------------------------------------------
These scripts are written for the MSc Thesis of Alex van den Berg,
Titled: "Visual Feedback for Haptic Shared Control Enabled Teleoperation of
an Industrial Robot - With Dross Removal as a Use Case""
Submitted in partial fulfillment for the requirements of my Master’s degree
in Biomechanical Design at the Delft University of Technology.
Student number: 4228391

Date: August 27, 2020, Delft, The Netherlands.
...
class DataProcessing(object):
    ...
    def checkAssumptions(self, dataList, df = []):  #  Check the assumptions for 2-way repeated measures ANOVA
        ...
        if shapirocsv:   #  a last-minute function used to print all the shapiro wilks test results to a separate CSV file so that they can easily be imported to latex
    ...
    def printAnovaToCSV(self, anova_table):  #  a last-minute function used to print all the anova results to a separate CSV file so that they can easily be imported to latex
...
if __name__ == "__main__":  # this part of the script is executed when this file is invoked directly
    ### Settings
    dataMainDir = "./../01 Raw Data/01 Recorded Data/"  # Path to the folder containing all the raw data
    doSwarm             = True      #  When this is on, the seperate datapoints are plotted on the boxplots
    plotqq              = False     #  When this is turned on, the qqplots are made for all the datasets
    doCheckInteractions = True      #  When this is turned on, the interactions are checked, if a significant interaction effect is found. This will perform a post-hoc analysis, and make the interaction plots
    doTransformation    = True      #  When this is turned on, the transformed datasets are used if the data does not pass the shapiro wilks test of normality.
    # NOTE ON TRANSFORMATIONS: if data needs to be transformed, it needs to be done using the ARTool in \00 Data Storage\02 Analysis Data\ARToolExe,
    ...
    # This needs to be done only once, and needs to be redone when the actual datasets, or calculations of the results have changed.

    # The logger has two settings, change it to debug if you want more detailed console outputs.
    loggingLevel = logging.INFO  # DEBUG, INFO

    ### Plot all the data of the experiment
    DP = DataProcessing(dataMainDir, doSwarm, plotqq, doTransformation, doCheckInteractions, loggingLevel)

    #DP.printFinalQ(3)  #  print the results of the final questionaire (unedited responses)  in the console, argument can be 0,1,2,3 - Commented out now because else it fills up the console window

    includeFails = True
    DP.plotNScoops(includeFails)
    DP.plotAvgScoopSize(includeFails)
    DP.plotvdLaan()
    DP.plotScoopFlux(includeFails)
    DP.plotProximity(includeFails)
    DP.plotDross(includeFails)
    DP.plotDisturbance(includeFails)
    DP.plotAvgVel(includeFails)
    DP.plotCollision(includeFails)
    DP.plotAccuracy(includeFails)
    DP.plotFailure()
    DP.plotPreference()

    ### Plot trial data  - These are examples on how to plot the raw data using these scripts
    #DP.timePlot(1,"obi",["bath"],[1,2],[0])
    #DP.timePlot(1,"RB",["mag"],[1,2],[0])  #  Some of the data still needs some filtering...
    #DP.timePlot(1,"proximity",["mag"],[1,2],[0])
    #DP.timePlot(1,"omni",["mag"],[1,2],[0])
    ### This is a relatively simple function I wrote to calculate the correlation between the datasets, if that would be of interest to anyone
    #DP.checkCorrelations(PS.plotDross(includeFails),PS.plotDisturbance(includeFails))